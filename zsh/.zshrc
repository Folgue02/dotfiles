...
export PATH=$HOME/go/bin:$HOME/.config/composer/vendor/bin:$HOME/.local/share/gem/ruby/3.3.0/bin/:$HOME/.pyenv/bin:$PATH:$HOME/scripts
...
alias sysupdate="yay -Syu --sudoloop --noconfirm"
...
alias ruby-lsp="/home/folgue/.rbenv/versions/3.3.0/lib/ruby/gems/3.3.0/gems/ruby-lsp-0.23.6/exe/ruby-lsp"
...
function goto() {
    if [ -z $1 ]; then
        echo "No path alias or subcommand specified."
        return 1
    fi
    ...
    if [ $ARG = "list" ] || [ $ARG = "edit" ]; then
        bris goto $ARG
        return 0
    fi

    OUTPUT=$(bris goto $ARG)
    EXIT_CODE=$?
